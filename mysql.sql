...
-- 餐位配置表
DROP TABLE IF EXISTS `seat_config`;
CREATE TABLE `seat_config`
(
    `id`               int                                                           NOT NULL AUTO_INCREMENT,
    `seat_type`        tinyint DEFAULT 0                                             NOT NULL COMMENT '餐位类型',
    `quantity`         int                                                           NOT NULL COMMENT '餐位数量',
    `create_time`      timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='餐位配置表';

-- 订餐表
DROP TABLE IF EXISTS `pre_dinner`;
CREATE TABLE `pre_dinner`
(
    `id`               bigint                                                        NOT NULL AUTO_INCREMENT,
    `user_id`              int                                                           NOT NULL COMMENT '用户id',
    `seat_id`          int DEFAULT 0                                                 NOT NULL COMMENT '餐位配置id',
    `status`           tinyint DEFAULT 0                                             NOT NULL COMMENT '用餐状态：0未使用 1使用中 2已使用 3已取消',
    `dinner_time`      timestamp                                                     DEFAULT NULL COMMENT '预定用餐时间',
    `create_time`      timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='订餐表';
