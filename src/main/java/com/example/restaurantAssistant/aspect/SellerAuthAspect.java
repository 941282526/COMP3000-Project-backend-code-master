...
import com.example.restaurantAssistant.constant.CookieConst;
import com.example.restaurantAssistant.constant.RedisConst;
import com.example.restaurantAssistant.enums.ResultEnum;
import com.example.restaurantAssistant.enums.SellerAdminRightsEnum;
...
import com.example.restaurantAssistant.service.SellerService;
import com.example.restaurantAssistant.utils.CookieUtil;
...
public class SellerAuthAspect {
    ...
    @Pointcut("execution(public * com.example.restaurantAssistant.controller.seller.Sell*.*(..))" +
            " && !execution(public * com.example.restaurantAssistant.controller.seller.SellerUserController.login(..))"+
            " && !execution(public * com.example.restaurantAssistant.controller.seller.SellerUserController.qrLogin(..))")
    public void checkLogin(){}


    //定义切面 拦截后台管理页面中对管理员数据接口的访问
    @Pointcut("execution(public * com.example.restaurantAssistant.controller.seller.SellerAdminController*.*(..))")
...
//        String token = request.getHeader("token");
        ...
        String sellerJson = stringRedisTemplate.opsForValue().get(String.format(RedisConst.TOKEN_PREFIX, cookie.getValue()));
